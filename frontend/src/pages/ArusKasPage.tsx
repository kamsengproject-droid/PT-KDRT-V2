...
export const ArusKasPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeTransactions(
      ...
      (data) => {
        /*
         * VOID tidak dihitung sebagai transaksi aktif.
         *
         * Historical transaction tanpa status tetap dianggap ACTIVE.
         */
         ...
         * IMPORTANT:
         *
         * Commission Real bukan uang bank.
         *
         * Jika ada transaksi lama dengan sourceType:
         * - COMMISSION_REAL
         * - TIKTOK_COMMISSION
         *
         * jangan tampilkan sebagai cashflow bank.
         *
         * Data tersebut tetap ada di Firestore,
         * tetapi tidak menjadi saldo Kas & Bank.
         */
        const bankTransactions = activeTransactions.filter((tx) => {
          if (
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      /*
       * Historical Commission Real transaction
       * menggunakan atomic delete.
       *
       * Transaksi baru seharusnya tidak lagi dibuat
       * dari Komisi Real, tetapi logic lama tetap
       * dipertahankan agar historical data tidak rusak.
       */
      if (
  ...
  const renderSection = (
    ...
    filtered.forEach((tx) => {
       ...
       * gunakan netAmount karena hanya dana bersih
       * yang benar-benar masuk rekening.
       */
      if (tx.type === 'INCOME') {
            ...
            <table className="w-full text-left text-xs text-zinc-600">
              ...
              <tbody className="divide-y divide-zinc-100">
                {filtered.length === 0 ? (
                  ...
                  filtered.map((tx) => {
                    /*
                     * Nilai Uang Masuk untuk transaksi:
                     *
                     * INCOME    = amount
                     * TRANSFER  = netAmount
                     */
      ...
      {role === 'INVESTOR' && (
        <div className="rounded-xl border border-zinc-200 bg-zinc-100 p-6 text-center">
          ...
          <p className="text-xs font-bold text-zinc-500">
            Arus Kas Pribadi tidak ditampilkan untuk
            Investor.
          </p>
        </div>
      )}
