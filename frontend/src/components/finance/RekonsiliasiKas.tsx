...
const UNKNOWN_ACCOUNT = 'Rekening Belum Ditentukan';

function normalizeAccountName(
  accountName?: string | null
): string {
  return accountName?.trim() || UNKNOWN_ACCOUNT;
}

function isExcludedFromBank(
  transaction: FinancialTransaction
): boolean {
  const source = String(transaction.sourceType || '').toUpperCase();
...
function getIncomeAmount(
  ...
  if (
    String(transaction.sourceType || '').toUpperCase() ===
    'FUND_TRANSFER'
  ) {
    return Number(transaction.netAmount) || 0;
  }
...
export const RekonsiliasiKas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeReconciliations =
      subscribeReconciliations(setReconciliations);
  ...
  const bankTransactions = useMemo(() => {
    return transactions.filter((transaction) => {
      if (isExcludedFromBank(transaction)) {
        return false;
      }

      if (
        selectedScope !== 'ALL' &&
        transaction.scope !== selectedScope
      ) {
        return false;
      }
      ...
      return (
        transaction.date &&
        transaction.date <= reconcileDate
      );
    });
  }, [
    transactions,
  ...
  const totalIncome = useMemo(() => {
    return bankTransactions
      ...
      .reduce(
        (sum, transaction) =>
          sum + getIncomeAmount(transaction),
        0
      );
  }, [bankTransactions]);
  ...
  const accountBalances = useMemo(() => {
    ...
    bankTransactions.forEach((transaction) => {
      const name = normalizeAccountName(
        transaction.accountName ||
          transaction.toAccount
      );
  ...
  const modalSystemBalance = useMemo(() => {
    if (!accountName.trim()) {
  ...
  const handleSave = async (
    ...
    try {
      ...
      const difference = modalDifference;
      ...
      await createReconciliation(
        {
          date: reconcileDate,
          periodLabel: formatBulanTahun(
            reconcileDate.substring(0, 7)
          ),
          scope:
            selectedScope === 'ALL'
              ? 'SHARING'
              : selectedScope,
          systemBalance: modalSystemBalance,
          actualBalance:
            Number(actualBalanceInput) || 0,
          difference,
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-zinc-200 bg-white p-12 text-center text-zinc-500">
        ...
        <p className="font-bold">
          Rekonsiliasi Kas & Bank hanya dapat
      ...
      <div className="flex flex-col justify-between gap-4 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-xs text-zinc-500">
            Pastikan saldo Kas & Bank di sistem
            sama dengan saldo rekening sebenarnya.
          </p>
        </div>
        ...
        <button
          ...
          className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-zinc-900 px-4 py-2.5 text-xs font-black text-white hover:bg-zinc-800"
        >
      ...
      <section>
        <div className="mb-3">
          ...
          <p className="text-xs text-zinc-500">
            Saldo dihitung dari transaksi Kas &
            Bank. Komisi Real tidak dihitung.
          </p>
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-zinc-100 px-6 py-4">
          ...
          <span className="text-xs font-bold text-zinc-400">
            {reconciliations.length} Rekaman
          </span>
        </div>
